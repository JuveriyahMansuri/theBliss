...
<?php
   ...
   //below is for pagination
    ...
    $record=mysqli_num_rows(mysqli_query($conn,"SELECT * FROM feedback;"));

     $pagi=ceil($record/$per_page);
 //ends here

    $sql=mysqli_query($conn,"SELECT f.feedback_id,f.feedback,f.feedback_date,p.name,u.user_name FROM feedback f INNER JOIN product p ON p.product_id=f.product_id INNER JOIN  user u ON f.customer_id=u.user_id ORDER BY f.feedback_date DESC LIMIT $start,$per_page;");
...
<!-- this is the main content place holder -->
 ...
 // the below code is to display database records 
    while($rows=mysqli_fetch_array($sql)){ 
       ...
       <td align="center" min="1"><?php echo $rows['feedback_id']; ?></td>
     
        <td align="center"><?php echo $rows['feedback']; ?></td>

        <td align="center"> <?php $d_date = date("d-m-Y", strtotime($rows['feedback_date'])); echo $d_date ;?></td>
        ...
        {
        ?> 
       <td><img style="height:100px;width:100px;" src="image/Feedback/<?php echo $r5['feedback_image'];?>" /></td>
