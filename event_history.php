                    ...
                    <div class="table-main table-responsive">
                        <table class="table">
                            ...
                            <tbody>
                                ...
                                $eid = $_GET['$eid'];
                                //echo $eid;
                                $sql = "SELECT * FROM `event_booking_detail` eb ,`event_booking` e  WHERE e.event_booking_id = '$eid' AND eb.event_booking_id = e.event_booking_id  ORDER BY event_booking_date DESC;";
                            ...
                            <td class="name-pr" align="center">
                                    ...
                                    <td  align="center">
                                        <a href="#">
                                            <?php $cordate = $row['event_date'];  $codate = date("d-m-Y", strtotime($cordate)); echo $codate; ?>
								</a>
                                    </td>
                                    ...
                                    <a href="#"> <?php $pid = $row['product_id'];
                                     ...
                                     <td class="name-pr" align="center">
                                        <a href="#"> <?php $pid = $row['product_id'];
                                            $product = mysqli_query($con,"SELECT `name`,`price`,`offer/discount_id` FROM product WHERE product_id = '$pid';");
                                            $pro = mysqli_fetch_array($product);
                                            $nm = $pro['name'];
                                            $pr = $pro['price'];
                                            $discount_id = $pro['offer/discount_id'];
                                            echo $nm; ?>
								</a>
                                    </td>
                                    <td align="center">
...
if($discount_id!=NULL)
{
    $get_discount = mysqli_query($con,"SELECT `discount` FROM `offer/discount` WHERE `offer/discount_id` = '$discount_id';");
    $get_discount = mysqli_fetch_assoc($get_discount);
    $dis_per = $get_discount['discount'];                                     
    $dprice = $row['eprice'] * $dis_per/100;
    $eprice = $row['eprice'] -  $dprice;
...
?>
                                 ...
                                 echo $eprice * $row['quantity']; ?> </p> </td>
                                  <td>
                                    <?php
                                       if($row['event_status'] == 'accepted'){
                               ...
                               <?php
                                 $total = $total + ($eprice * $row['quantity']);    
                            }
                                ?>
                                 </tr>
