...
$discountprice = 0;
$gt = 0;
//$odate = mysqli_query($con,"SELECT CAST( NOW() AS Date );");
//$odate = mysqli_fetch_array($odate);
//$ordate = implode($odate);
//$odate =  date("Y-m-d");

$ordate =  mysqli_query($con,"SELECT NOW();");
$ordate = mysqli_fetch_assoc($ordate);
$ordate=implode($ordate);
...
if(isset($_POST['placeorder'])){
     ...
     $area = $_POST["area"];
     $odate = $ordate;
    $sql3 = "INSERT INTO `order` (`order_date`,`delivery_address`,`payment_mode`,`total_amount`,`customer_id`,`area_pincode`) VALUES ('$odate','$deliveryadd','$paymode','$total','$cid','$area');";
    $res3 = mysqli_query($con , $sql3);
    //var_dump($res3);
    $query = "SELECT `order_id` FROM `order` WHERE `customer_id` = '$user'  AND `order_date` = '$odate';";
    $oid = mysqli_query($con, $query);
    $orid = mysqli_fetch_array($oid);
    echo $orid = $orid['order_id'];
    $product = mysqli_query($con , "SELECT * FROM Product p , Cart c WHERE p.product_id = c.product_id AND user_id = '$user';");
    while ($row1 = mysqli_fetch_array($product)){
        $pid = $row1['product_id'];
        $qty = $row1['quantity'];
        $price = $row1['price'];

        $QOH = $row1['quantity_in_hand']; 
        if($QOH!=null)
        {
            $stock = $QOH - $qty;
            $DQOH = mysqli_query($con,"UPDATE `product` SET quantity_in_hand = '$stock' WHERE product_id = '$pid';");
            //var_dump($DQOH);
        }
        else{
        ...
        //echo $orid; 
        $sql4 = "INSERT INTO `order_detail` (`order_id`,`product_id`,`quantity`,`price`,`discount_price`) VALUES ('$orid','$pid','$qty','$price','$discount');";
        $res4 = mysqli_query($con, $sql4);
        //var_dump($res4);
    }
    /*var_dump($res4);
    $results=mysqli_result(res4);
    $rowcount=mysqli_num_rows($results);
    if($rowcount != 0){*/
        if($res4){
            //echo "order Placed";
            $_SESSION['order_id'] = $orid;
            header("location: order_detail.php");   
        }
    $sql5 = mysqli_query($con,"DELETE  FROM Cart WHERE user_id = '$user';");
...
?>
                            ...
                            <div class="shipping-method-box">
                              ...
                              <!--  <label for="paymode">Payment mode *</label>
                                    <select class="wide w-100" id="paymode" name="paymode" required>
                                        <option value="payment mode" data-display="Select">Payment mode</option>
                                   ...
                                   <!-- <div class="custom-control custom-radio">
                                        <input id="shippingOption1" name="shipping-option" class="custom-control-input"  type="radio">
                                        <label class="custom-control-label" for="Cash on Delivery">Cash on Delivery</label> </div>
                                    <div class="custom-control custom-radio">
                                        <input id="shippingOption2" name="shipping-option" class="custom-control-input" type="radio">
                                        <label class="custom-control-label" for="Online">Online</label> </div>-->
                            </div>
            </div>
                ...
                <div class="col-sm-6 col-lg-6 mb-3">
                    <div class="row">
                        ...
                        <div class="col-12 d-flex shopping-box"> <button type="submit" style ="color:white" class="ml-auto btn hvr-hover"  name="placeorder"> <b> Place Order  </b></button> </div>
                    </div>
                </div>
            </div>
